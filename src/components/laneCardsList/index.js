    ...
    return (
      <div className="lane-cards-list">
        ...
        <div className="lane-cards-list-tabs">
          ...
          <Tab
            onClick={() => {
              this.handleTabClick("isOutbid");
            }}
            selected={filter === "isOutbid"}
            text="outbid"
            badge={getOutbidCount(lanes)}
            badgeColor="#e76f67"
          />
          <Tab
            onClick={() => {
              this.handleTabClick("isWatching");
            }}
          ...
          <Tab
            onClick={() => {
              this.handleTabClick("isWinning");
            }}
            selected={filter === "isWinning"}
            text="winning"
            badge={getWinningCount(lanes)}
          />
        </div>
        {lanes.filter(filterLanes(filter)).map(lane => (
          <LaneCard
            onClick={onCardClick}
            key={`${lane.lane}-${lane.auction}`}
            {...lane}
          />
        ))}
      </div>
...
const getOutbidCount = lanes =>
  lanes.reduce((total, lane) => (lane.isOutbid ? total + 1 : total), 0);

const getWinningCount = lanes =>
  lanes.reduce((total, lane) => (lane.isWinning ? total + 1 : total), 0);
